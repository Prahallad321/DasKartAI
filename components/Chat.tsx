        ...
        <div className="w-full absolute bottom-0 z-20 px-4 pb-4 md:pb-6 pt-10 bg-gradient-to-t from-slate-950 via-slate-950/90 to-transparent pointer-events-none">
                ...
                <div className="relative bg-slate-900 rounded-[26px] shadow-2xl border border-slate-800">
                    {showAttachMenu && (
                        <div ref={menuRef} className="absolute bottom-full left-0 mb-2 ml-2 bg-slate-800 border border-slate-700 rounded-xl shadow-xl min-w-[180px] z-50">
                            ...
                            <button onClick={() => { setInputValue('Research: '); setShowAttachMenu(false); inputRef.current?.focus(); }} className="flex items-center gap-3 w-full px-4 py-3 hover:bg-slate-700 text-left text-sm text-slate-200"><Globe size={18} className="text-cyan-400" />Deep Research</button>
                            <button onClick={() => { setInputValue('Shopping Research: '); setShowAttachMenu(false); inputRef.current?.focus(); }} className="flex items-center gap-3 w-full px-4 py-3 hover:bg-slate-700 text-left text-sm text-slate-200"><ShoppingBag size={18} className="text-pink-400" />Shopping Research</button>
            ...
            {attachments.length > 0 && (
                <div className="absolute bottom-full left-0 w-full px-4 pb-2">
                    <div className="max-w-3xl mx-auto flex gap-2 overflow-x-auto py-2">
                         {attachments.map((att, i) => (
                             <div key={i} className="relative group flex-shrink-0">
                                 {att.mimeType.startsWith('image/') ? (
                                     <img src={`data:${att.mimeType};base64,${att.data}`} className="h-16 w-16 object-cover rounded-lg border border-slate-700" alt="attachment" />
                                 ) : (
                                     <div className="h-16 w-16 bg-slate-800 rounded-lg flex items-center justify-center border border-slate-700"><FileText size={24} className="text-slate-400" /></div>
                                 )}
                                 <button onClick={() => removeAttachment(i)} className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full p-0.5 shadow-md"><X size={12} /></button>
                             </div>
                         ))}
                    </div>
                </div>
